...
## Character editor catalog

The optional **Character Editor** tab exports a module-declared catalog under `metadata.characterEditor`. It is deliberately metadata, not the deprecated top-level `capabilities` shape. A consuming editor uses it to show only visual controls that the module explicitly supports:
